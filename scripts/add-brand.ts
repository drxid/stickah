...
async function main() {
  ...
  const picked = only.length ? brand.flavors.filter((f) => only.some((q) => matches(f, q))) : brand.flavors
  const pickedLines = new Set(picked.map((f) => f.lineId))

  const current = JSON.parse(await readFile(file, 'utf8')) as Catalog
  const newLines = brand.lines.filter((l) => pickedLines.has(l.id)).filter(missingFrom(current.lines))
  ...
  const merged: Catalog = {
    ...current,
    manufacturers: [...current.manufacturers, ...brand.manufacturers.filter(missingFrom(current.manufacturers))],
