...
export function validateCatalog(c: Catalog): string[] {
  ...
  for (const l of c.lines) {
    if (!mIds.has(l.manufacturerId))
      errors.push(`линейка ${l.id}: неизвестный производитель ${l.manufacturerId}`);
  }
  for (const f of c.flavors) {
    if (!f.name?.trim()) errors.push(`вкус ${f.id}: пустое имя`);
    if (!mIds.has(f.manufacturerId))
      errors.push(`вкус ${f.id}: неизвестный производитель ${f.manufacturerId}`);
