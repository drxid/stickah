...
function grid(x: number, y: number, w: number, h: number, cell: Cell, gutter: number): Slot[] {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      slots.push({ x: x + c * (cell.w + gutter), y: y + r * (cell.h + gutter), rotated: cell.rotated })
    }
  }
  return slots
}
...
export function computeLayout(size: SizePreset, cfg: LayoutConfig = DEFAULT_LAYOUT): SheetLayout {
  ...
  let slots = pack(0, 0, A4.width - 2 * cfg.margin, A4.height - 2 * cfg.margin, cells, cfg.gutter, PACK_DEPTH)
